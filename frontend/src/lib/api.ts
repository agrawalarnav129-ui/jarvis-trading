...
export const api = {
  ...
  paper: () => get<{ available: boolean; note?: string; portfolios: { name: string; started: string; equity: number; return_pct: number; trades: number; open_positions: number; win_rate: number; total_r: number; open_r: number; avg_r: number; max_drawdown: number; open: any[]; closed: any[]; curve: { date: string; equity: number }[] }[] }>("/api/paper"),
  earningsPlaybook: (symbol: string) => get<{ available: boolean; note?: string; symbol?: string; n?: number; avg_abs_move?: number; max_abs_move?: number; pct_up?: number; avg_drift5?: number; last_move?: number; next_date?: string; days_to?: number }>(`/api/earnings-playbook?symbol=${encodeURIComponent(symbol)}`),
