...
export default function Assistant() {
  ...
  const send = () => {
    ...
    setMsgs((m) => [...m, { role: "user", text: input },
      { role: "axiom", text: "AI endpoint not wired yet — this is the next migration step. Your Groq-powered brain runs on the backend; I'll connect /api/assistant next." }]);
    setInput("");
  };
      ...
      <Card className="min-h-[55vh] flex flex-col">
        ...
        <div className="flex gap-2">
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()}
            placeholder="Ask AXIOM…" aria-label="Message AXIOM"
            className="flex-1 bg-base border border-line rounded-lg px-3 py-2 text-sm text-txt placeholder:text-faint outline-none focus:border-brand/60 transition-colors" />
          <button onClick={send} aria-label="Send"
            className="rounded-lg bg-brand/15 border border-brand/40 px-3.5 text-brand cursor-pointer hover:bg-brand/25 transition-colors">
            <Send size={16} />
          </button>
        </div>
      </Card>
